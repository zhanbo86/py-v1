...
#        image = tf.random_crop(image, [54, 54, 3])
    ...
    def build_batch(path_to_tfrecords_file, num_examples, batch_size, shuffled,num_epoch_):
        ...
        image,digits = Donkey._read_and_decode(filename_queue)

        min_queue_examples = int(0.4 * num_examples)
        if shuffled:
            image_batch, digits_batch = tf.train.shuffle_batch([image, digits],
                                                               batch_size=batch_size,
                                                               num_threads=2,
                                                               capacity=min_queue_examples + 3 * batch_size,
                                                               min_after_dequeue=min_queue_examples)
        else:
            image_batch, digits_batch = tf.train.batch([image,digits],
                                                        batch_size=batch_size,
                                                        num_threads=2,
                                                        capacity=min_queue_examples + 3 * batch_size)
        return image_batch,digits_batch
