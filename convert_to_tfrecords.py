...
#            attrs[key] = values
        ...
        item = f['digitStruct']['bbox'][index].item()
        for key in ['digit1', 'digit2', 'digit3', 'digit4']:
            attr = f[item][key]
...
#        label_of_digits = attrs['label']
        label_of_digits = attrs['digit1'],attrs['digit2'],attrs['digit3'],attrs['digit4']
...
#        digits = [10, 10, 10, 10, 10]   # digit 10 represents no digit
        digits = [11, 11, 11, 11]   # digit 11 represents no digit
        for idx, label_of_digit in enumerate(label_of_digits):
            digits[idx] = int(label_of_digit if label_of_digit != 11 else 0)    # label 11 is essentially digit zero
...
#        print(digits)
        image = np.array((Image.open(path_to_image_file)).resize([54, 54])).tobytes()
#        image = (Image.open(path_to_image_file)).resize([72, 36])
...
def convert_to_tfrecords(path_to_dataset_dir_and_digit_struct_mat_file_tuples,
    ...
    for path_to_dataset_dir, path_to_digit_struct_mat_file in path_to_dataset_dir_and_digit_struct_mat_file_tuples:
        path_to_image_files = tf.gfile.Glob(os.path.join(path_to_dataset_dir, '*.bmp'))
