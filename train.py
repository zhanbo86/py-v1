...
#    num_steps_to_check = 1000
    ...
    with tf.Graph().as_default():
        image_batch,digits_batch = Donkey.build_batch(path_to_train_tfrecords_file,
                                                                     num_examples=num_train_examples,
                                                                     batch_size=batch_size,
                                                                     shuffled=True,num_epoch_=training_options['num_epoch'])
        digits_logits = Model.inference(image_batch, drop_rate=0.2)
        loss = Model.loss(digits_logits,digits_batch)
